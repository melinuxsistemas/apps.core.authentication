...
class User(AbstractBaseUser):
    email             = models.EmailField(_('Email'), max_length=255, unique=True, null=True, validators=[email_format_validator, email_dangerous_symbols_validator], error_messages=settings.ERRORS_MESSAGES)
    username          = models.CharField("Username", max_length=20, unique=True, null=True, validators=[email_format_validator, email_dangerous_symbols_validator], error_messages=settings.ERRORS_MESSAGES)
    type_user         = models.CharField("Tipo de Usuário", max_length=1, null=False, choices=options_users, default='FUN', error_messages=settings.ERRORS_MESSAGES)
    joined_date       = models.DateTimeField(null=True, auto_now_add=True)
    last_access       = models.DateTimeField(null=True, auto_now=True)

    is_active         = models.BooleanField(default=True)
    account_activated = models.BooleanField(default=False)
    activation_code   = models.CharField(max_length=46, null=True, blank=True, error_messages=settings.ERRORS_MESSAGES)
    ...
    class Meta:
        db_table = 'core_authentication_users'
